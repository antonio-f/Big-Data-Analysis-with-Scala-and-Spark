...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val col_fields = StructField(columnNames.head, StringType, nullable = false) +: columnNames.tail.map(StructField(_, DoubleType, nullable = false));
    StructType(col_fields)
  }
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when('teage.between(15, 22), "young") when('teage.between(23, 55), "active") otherwise "elder" as "age"
    val ageProjection: Column = when('tesex === 1.0, "male") otherwise "female" as "sex"
